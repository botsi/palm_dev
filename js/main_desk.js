		...
		if (xmlhttp.readyState == 4) {
			...
			if (xmlhttp.status == 200) {
				...
				for (var key in obj) {
					...
					var a = (isNaN(key)) ? 'href="' + u + key + '.pdf" download="' + folders[chapter].data[folders[chapter].last_position].name.replace(/\./g, "").replace(/\!/g, "").replace(/\?/g, "").replace(/\, /g, "_").replace(/ \- /g, "_").replace(/ /g, "_") + '_' + obj[key][1].replace(/\, /g, "_").replace(/ /g, "_") + '_' + obj[key][2].replace(/ /g, "_") + '.pdf"' : 'onclick="parent_media(this,' + key + ')"';
...
var hit_menue = function(t, ju) {
	...
	if (typeof folders[chapter].data[0].bilder === 'undefined') {
		...
		cc.classList.add('palma_form_open');
		document.getElementById('header').classList.add('palma_form_open');
		document.getElementById('logo').classList.add('palma_form_open');
		document.getElementById('content').classList.add('palma_form_open');
...
var arrow = function(e) {
	...
	if (e.keyCode == 70 && meta_presed) {

		console.log('meta_presed (on f key): ', meta_presed);
		...
		if (cm.style.display != 'block') {
			...
			var th = document.getElementsByClassName('items')[0];

			cm.innerHTML = folders[th.ix].menu_ih;

			if (folders[th.ix].menu_ih != '') {
				...
				var ml = parseInt(th.getBoundingClientRect().left - cm.offsetWidth / 2 + this.offsetWidth / 2);
	...
	meta_presed = (e.metaKey);

	console.log('meta_presed (always): ', meta_presed);
